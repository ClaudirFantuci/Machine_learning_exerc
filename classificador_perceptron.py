...
def executar():
    ...
    for i, seed in enumerate(SEEDS, start=1):
        ...
        # Perceptron exige normalização para convergir corretamente
        scaler   = StandardScaler()
        X_treino = scaler.fit_transform(X_treino)
        X_teste  = scaler.transform(X_teste)

        modelo = Perceptron(max_iter=1000, random_state=0)
        modelo.fit(X_treino, y_treino)
        predicoes = modelo.predict(X_teste)

        acc = accuracy_score(y_teste, predicoes)
        cm  = confusion_matrix(y_teste, predicoes)
        rep = classification_report(y_teste, predicoes, output_dict=True, zero_division=0)
    ...
    df_res.to_csv("resultado_perceptron.csv", index=False)
    ...
    print(f"  Resultados salvos em: resultado_perceptron.csv")